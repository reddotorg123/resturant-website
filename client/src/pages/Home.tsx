...
function Nav() {
  ...
  return <>
    <header className={`site-nav ${scrolled ? "scrolled" : ""} ${hidden ? "hidden-nav" : ""}`}>
      <Link href="/" className="wordmark" aria-label="Ember and Stone home">E / S</Link>
    ...
    {open && <div className="mobile-overlay">
      <div className="mobile-top"><span className="wordmark">E / S</span><button className="menu-toggle" onClick={() => setOpen(false)} aria-label="Close menu"><X size={26} /></button></div>
      ...
      <div className="mobile-foot"><span>17 Mercer Street</span><span>Lower Manhattan</span></div>
    </div>}
  </>;
}

function Marquee() {
  return <div className="marquee-wrap"><div className="marquee">{Array.from({ length: 2 }).map((_, index) => <span key={index}>Season-led cooking <span>New York, NY</span> Open Tuesday — Saturday <span>Fire / Ferment / Gather</span></span>)}</div></div>;
}
...
function Hero() {
  return <section className="hero" id="top">
    ...
    <div className="hero-content">
      <div className="hero-kicker eyebrow">Lower Manhattan · New York</div>
...
function HomePage() {
  return <main>
    ...
    <section id="arrival" className="section section-dark arrival">
      <Reveal className="arrival-visual"><img src="https://images.unsplash.com/photo-1498654896293-37aacf113fd9?auto=format&fit=crop&w=1600&q=88" alt="Candlelit table at Ember and Stone" loading="lazy" /><div className="image-caption eyebrow"><span>Chapter 01</span><span>Arrival</span></div></Reveal>
      <Reveal className="arrival-copy" delay={1}><div className="eyebrow" style={{ color: "var(--copper)" }}>The room</div><h2 className="display">Come in.<span>Discover.</span></h2><p className="section-copy">Past the street, the room gets quieter. A long table, low light, the scent of something just kissed by flame. We built Ember & Stone as a pause from the city — a place to linger between courses.</p><a className="link-arrow" href="#cuisine">Follow the light <ArrowRight size={15} /></a></Reveal>
    </section>
...
function SmallPage({ kind }: { kind: "contact" | "events" | "private" }) {
  ...
  return <main><PageHero eyebrow={data.eyebrow} title={data.title} image={data.image} /><section className="section section-cream"><div className="section-intro"><Reveal><div className="eyebrow">A note from Ember & Stone</div></Reveal><Reveal delay={1}><p className="display" style={{ fontSize: "clamp(2.7rem, 6vw, 6rem)", lineHeight: .88, margin: 0 }}>{data.body}</p><Link className="link-arrow" style={{ marginTop: 48 }} href={kind === "private" ? "/reservations" : "/location"}>{kind === "private" ? "Enquire now" : "Get in touch"} <ArrowRight size={15} /></Link></Reveal></div></section><FinalCTA /></main>;
}

function Footer() {
  return <footer className="footer"><div className="footer-grid"><div><div className="eyebrow" style={{ color: "var(--copper)" }}>Ember & Stone</div><h2 className="display footer-title">Good food.<br />Good company.<br /><span style={{ color: "var(--copper)" }}>Good night.</span></h2></div><div className="footer-column"><div className="eyebrow">Explore</div><Link href="/menu">Menu</Link><Link href="/about">About</Link><Link href="/experience">Experience</Link><Link href="/gallery">Gallery</Link></div><div className="footer-column"><div className="eyebrow">Visit</div><Link href="/reservations">Reservations</Link><Link href="/location">Location</Link><Link href="/private-dining">Private dining</Link></div><div className="footer-column"><div className="eyebrow">Contact</div><span>{restaurantConfig.address}</span><span>{restaurantConfig.hours}</span><a href={`mailto:${restaurantConfig.email}`}>{restaurantConfig.email}</a></div></div><div className="footer-bottom"><span>© 2026 Ember & Stone</span><span>A study in fire, season, and the spaces between.</span></div></footer>;
}
